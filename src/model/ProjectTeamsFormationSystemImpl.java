...
public class ProjectTeamsFormationSystemImpl implements ProjectTeamsFormationSystem {
	...
	public List<Project> getPopularProjects() {
		...
		return (List<Project>) connection.getPopularProjects(idealNumberOfProjects);
	}
	...
	public boolean swap(Student s1, Student s2, int acceptableChange) {
		...
		if (!((project1.getId()).equals(project2.getId()))) {
			...
			if ((validator.calculateFit(temp1) >= acceptableChange)
					&& (validator.calculateFit(temp2) >= acceptableChange)) {
	...
	/**
	 * assign the given student to a project based on his preferences and
	 * constraints
	 * 
	 * @param student
	 * @param popularProjects
	...
	public void assign() {
		...
		List<Student> femaleStudents = (List<Student>) connection.getFemaleStudents();
		List<Student> maleStudents = (List<Student>) connection.getMaleStudents();
	...
	@Override
	public boolean assignAllStudents()
			throws InsufficientProjectsException, NoStudentException, RemainedStudentsException {
